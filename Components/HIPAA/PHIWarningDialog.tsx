import React from 'react';
import styles from './HIPAACompliance.module.css';

interface PHIWarningDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onProceed: () => void;
  detectedPHI: string[];
  messagePreview?: string;
  severity?: 'low' | 'medium' | 'high';
}

const PHIWarningDialog: React.FC<PHIWarningDialogProps> = ({ 
  isOpen, 
  onClose, 
  onProceed, 
  detectedPHI,
  messagePreview,
  severity = 'medium'
}) => {
  if (!isOpen) return null;

  return (
    <div className={styles.dialogOverlay}>
      <div className={styles.dialogContent}>
        <h3>⚠️ PHI Detection Warning</h3>
        <div className={styles.warningContent}>
          <p>Potential Protected Health Information (PHI) detected in your message:</p>
          <ul>
            {detectedPHI.map((item, index) => (
              <li key={index} className={styles[`severity-${severity}`]}>{item}</li>
            ))}
          </ul>
          {messagePreview && (
            <div className={styles.messagePreview}>
              <p>Message Preview:</p>
              <pre>{messagePreview}</pre>
            </div>
          )}
          <div className={styles.warningInfo}>
            <p>Please confirm that you:</p>
            <ul>
              <li>Have necessary authorization to share this information</li>
              <li>Are sending to an authorized recipient</li>
              <li>Are aware this will be logged in the audit trail</li>
              <li>Understand the implications of sharing PHI</li>
            </ul>
          </div>
        </div>
        <div className={styles.dialogButtons}>
          <button 
            className={styles.cancelButton} 
            onClick={onClose}
          >
            Edit Message
          </button>
          <button 
            className={`${styles.proceedButton} ${styles[`severity-${severity}`]}`}
            onClick={onProceed}
          >
            Proceed Securely
          </button>
        </div>
      </div>
    </div>
  );
};

export default PHIWarningDialog;
